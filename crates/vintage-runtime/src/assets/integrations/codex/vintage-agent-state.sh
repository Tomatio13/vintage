...
# Installed by VINTAGE. Reports the native session id to VINTAGE's hook IPC.
# Does nothing when the VINTAGE hook environment is absent. Sends over TCP via
# node (works on any OS with node installed); no python dependency.
set -eu
...
hook_input_file="$(mktemp "${TMPDIR:-/tmp}/VINTAGE-codex-hook.XXXXXX")" || exit 0
...
const agent = process.env.VINTAGE_AGENT || "codex";
