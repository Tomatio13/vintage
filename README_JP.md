...
<p align="center">
  <img src="./docs/assets/vintage-main.png" width="100%" alt="ワークスペースのサイドバー、分割ターミナル、ファイルエディターを表示したVINTAGEのデスクトップ画面">
</p>
...
- **作業ファイルをその場で扱えます。** 更新を監視するファイルツリーから、テキストファイルを編集・保存し、画像、PDF、フォントをプレビューできます。
...
| ファイル | 隠しファイル表示、テキスト編集・保存、ファイルマネージャー操作、各種プレビュー |
...
1. 右側の**Files**を開き、ワークスペースのテキストファイルを編集・保存し、その他のファイルをプレビューします。
...
- **Zsh (fast startup)**は、<code>zsh -f</code>でZshの起動ファイルの大半を読み込みません。テーマ依存のプロンプトを継承せず、ユーザー名・ホスト名・ディレクトリの組み込みプロンプトを使います。エージェント終了後も同じ表示になります。
...
Reactレンダラーは、プロセスの起動やファイルシステムへの直接アクセスを行いません。シェル検出、実行ファイルの検証、PTYの生成と終了、ワークスペース登録、ファイル監視、ファイルの読み書き、プレビューをRustホストが担当します。Hook／Plugin連携の基盤も、ローカルIPCサーバー、トークン検証、資産管理をすべてRustホストに置いています。
...
- テキストの編集・保存上限は512 KiB、フォントのプレビュー上限は8 MiB、対応する画像とPDFのプレビュー上限は20 MiBです。
